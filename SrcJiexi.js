let parse = {
    ...
    分类: function () {
        ...
        posterCards.forEach(function (item) {
            ...
            d.push({
                ...
                url: url,
                extra: {
                    url: "hiker://empty#" + cid + "#" + (p.vid || "") + "#immersiveTheme#",
                    pic: pic || "",
                    title: p.title || ""
                }
            });
    ...
    二级: function (url) {
        ...
        const title = MY_PARAMS && MY_PARAMS.title ? MY_PARAMS.title : "";
        const image = MY_PARAMS && MY_PARAMS.pic ? MY_PARAMS.pic : "";
        ...
        /*
            ...
            const videoApiUrl = "https://pbaccess.video.qq.com/trpc.universal_backend_service.page_server_rpc.PageServer/GetPageData";
            ...
            // 获取并处理剧集数据
            const fetchAndProcessEpisodes = (context = "", isTabPage = false) => {
              let body = JSON.parse(JSON.stringify(episodeBody)); // 深拷贝
              if (context) {
                body.page_params.page_context = context;
              }
        
              let episodeData = JSON.parse(
                fetch(videoApiUrl, {
                  headers: {
                    "Content-Type": "application/json",
                    "referer": "https://v.qq.com/",
              ...
              if (episodeData.data && episodeData.data.module_list_datas && episodeData.data.module_list_datas.length > 0) {
                let module_datas = episodeData.data.module_list_datas[0].module_datas[0];
        
                // 处理当前页剧集数据
                if (module_datas && module_datas.item_data_lists && module_datas.item_data_lists.item_datas) {
                  let list_Json = module_datas.item_data_lists.item_datas;
        
                  list_Json.forEach(item => {
                    if (item.item_params.is_no_store_watch_history != 1) {
                      try {
                        let pic = item.item_params.image_url ? item.item_params.image_url + "@Referer=" : "";
                        let itemCid = item.item_params.cid || "";
                        let vid = item.item_params.vid || "";
        
                        const episodeTitle = (item.item_params.play_title || "").split(" ").slice(-1)[0];
                        if (episodeTitle && pic) {
                          episodes.push({
                            title: episodeTitle,
                            img: pic,
                            desc: this.secondsToTime(item.item_params.duration),
                            url: `https://v.qq.com/x/cover/${itemCid}/${vid}.html`,
                            col_type: "movie_2"
                          });
                        }
                      } catch (e) {
                ...
                if (!isTabPage) {
                  // 处理分页数据
                  let next_page_context = module_datas.module_params.next_page_context;
                  let has_next = module_datas.module_params.has_next;
        
                  // 使用并发请求获取所有分页数据
                  const contexts = [];
                  let pageCount = 0;
                  while (has_next === "true" && pageCount < 10) {
                    contexts.push(next_page_context);
        
                    // 获取下一页信息但不处理数据，只为了获得后续页的上下文
                    let nextPageInfo = JSON.parse(
                      fetch(videoApiUrl, {
                        headers: {
                          "Content-Type": "application/json",
                          referer: "https://v.qq.com/",
                    ...
                    if (nextPageInfo.data && nextPageInfo.data.module_list_datas && nextPageInfo.data.module_list_datas[0] && nextPageInfo.data.module_list_datas[0].module_datas) {
                      let nextPageModule = nextPageInfo.data.module_list_datas[0].module_datas[0];
                      next_page_context = nextPageModule.module_params.next_page_context;
                      has_next = nextPageModule.module_params.has_next;
                    } else {
                      break;
                    }
                  ...
                  if (contexts.length > 0) {
                    contexts.forEach(context => {
                      ...
                      if (pageData.data && pageData.data.module_list_datas && pageData.data.module_list_datas[0] && pageData.data.module_list_datas[0].module_datas) {
                        let pageModule = pageData.data.module_list_datas[0].module_datas[0];
        
                        if (pageModule && pageModule.item_data_lists && pageModule.item_data_lists.item_datas) {
                          let pageList = pageModule.item_data_lists.item_datas;
            ...
            // 处理初始剧集列表
            let firstModule = fetchAndProcessEpisodes();
        
            // 处理tabs数据（不同季度或版本），使用并发处理
            if (firstModule) {
              let module_params_tabs = firstModule.module_params.tabs;
              if (module_params_tabs) {
                try {
                  ...
                  const tabContexts = [];
                  // 限制处理的标签页数量，最多处理5个
                  const tabsLimit = Math.min(module_params_tabs.length, 6);
                  for (let i = 1; i < tabsLimit; i++) {
                    tabContexts.push(module_params_tabs[i].page_context);
                  }
        
                  // 并发处理所有标签页
                  tabContexts.forEach(context => {
                    fetchAndProcessEpisodes(context, true);
                  });
                } catch (e) {
                  log("处理tabs数据失败: " + e.message);
                }
              }
            }
        */
        ...
        function getVideoList(input, firt) {
            ...
            item_datas.forEach(it => {
                选集.push({
                    ...
                    desc: it.item_params.video_subtitle,//parse.secondsToTime(it.item_params.duration),
        ...
        return {
            ...
            img: image,
            list: tab_list.选集,
            page: tab_list.分页.length==0?undefined:tab_list.分页,
            pageparse: tab_list.分页.length==0?undefined:getVideoList
        };
    },
                        ...
                        stream.itemList.forEach(item => {
                            if (item && item.videoInfo && item.doc) {
                                ...
                                searchResults.push({
                                    ...
                                    extra: {
                                        pic: (videoInfo.imgUrl || (coverDoc && coverDoc.imgUrl) || "") + "@Referer=",
                                        detail1: (info.actors ? "主演: " + (Array.isArray(info.actors) ? info.actors.slice(0, 3).join(",") : info.actors) : ""),
