...
function getYiData(datatype, jkdata, dd) {
    ...
    try {
        ...
        xlog('获取'+datatype+'数据完成，耗时：' + (t2-t1) + 'ms');
    } catch (e) {
...
function ocr(codeurl,headers) {
    ...
    let code = request('https://api.alistgo.com/ocr/b64/text', { body: img, method: 'POST', headers: {"Content-Type":"text/html"}});
...
// 请求返回html源码
function getHtmlCode(ssurl, headers, timeout) {
...
// 打造动态分类
function buildDynCate(html, 定位列表){
