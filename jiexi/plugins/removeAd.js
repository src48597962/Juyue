...
function cleanM3u8RemoveAds(m3u8Content) {
    ...
    const totalSegments = lines.filter(l => l.endsWith('.ts')).length;
    if (totalSegments < 10) {
        log('✅ 切片过少，不执行广告清理');
        return m3u8Content;
    }
...
function markAdsSafely(segments, discontinuities) {
    ...
    const sampleSize = Math.max(8, Math.floor(segCount * 0.2));
    const sample = segments.slice(0, sampleSize);

    // 提取正片标准特征
    const avgNameLength = sample.map(s => s.filename.length).reduce((a, b) => a + b, 0) / sampleSize;
    const isHexNormal = name => /^[0-9a-f]{30,40}/.test(name); // 视频站正片通用规则
    ...
    segments.forEach((seg, idx) => {
        ...
        // 特征1：文件名长度突变（广告最典型）
        const lenDelta = Math.abs(name.length - avgNameLength);
        if (lenDelta > 8) score += 35;

        // 特征2：命名规则突变（正片是32位哈希，广告不是）
        const normalName = isHexNormal(name);
        if (!normalName) score += 40;

        // 特征3：时长剧烈波动 >2秒
        const prev = segments[idx > 0 ? idx - 1 : 0];
        const durDelta = Math.abs(seg.duration - prev.duration);
